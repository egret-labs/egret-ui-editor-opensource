...
export class ResourceMap<T> {
	...
	public set(resource: URI, value: T): void {
		if (this.keys.indexOf(resource) == -1) {
			this.keys.push(resource);
		}
	...
	public delete(resource: URI): boolean {
		const index = this.keys.indexOf(resource);
		if (index != -1) {
	...
	public forEach(clb: (key: URI, value: T) => void);
	public forEach(clb: Function): void {
		if (clb.length == 1) {
			this.map.forEach(clb as (value: T) => void);
		} else if (clb.length == 2) {
			this.keys.forEach((resource: URI) => {
				clb(resource, this.get(resource));
			});
		}
	}
