 ...
 */
export function trim(haystack: string, needle: string = null): string {
 ...
 */
export function ltrim(haystack?: string, needle: string = null): string {
	if (!haystack) {
		return '';
	}

	if (!needle) {
		let char: string = haystack.charAt(0);
		while (haystack.length > 0 &&
			(char === ' ' || char === '\t' || char === '\n' || char === '\r' || char === '\f')) {
			haystack = haystack.substr(1);
			char = haystack.charAt(0);
		}
		return haystack;
	}
	else {
 ...
 * @param needle 要去除的字符，null表示不可见字符
 */
export function rtrim(haystack?: string, needle: string = null): string {
	if (!haystack) {
		return '';
	}

	if (!needle) {
		let char: string = haystack.charAt(haystack.length - 1);
		while (haystack.length > 0 &&
			(char === ' ' || char === '\t' || char === '\n' || char === '\r' || char === '\f')) {
			haystack = haystack.substr(0, haystack.length - 1);
			char = haystack.charAt(haystack.length - 1);
		}
		return haystack;
	}
	else {
		const needleLen: number = needle.length;
		const strLen: number = haystack.length;
