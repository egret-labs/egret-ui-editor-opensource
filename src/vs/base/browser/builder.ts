...
const MS_DATA_KEY = '_msDataKey';
const DATA_BINDING_ID = '__$binding';
const LISTENER_BINDING_ID = '__$listeners';
const VISIBILITY_BINDING_ID = '__$visibility';
...
export class Builder implements IDisposable {
	...
	public showDelayed(delay: number): Builder {
		...
		const promise = TPromise.timeout(delay);
	...
	private cancelVisibilityPromise(): void {
		const promise: TPromise<void> = this.getProperty(VISIBILITY_BINDING_ID);
		if (promise) {
			promise.cancel();
			this.removeProperty(VISIBILITY_BINDING_ID);
		}
	...
	public selectOne(selector: string, offdom?: boolean): Builder {
		assert.ok(types.isString(selector), 'Expected String as parameter');

		const element = this.currentElement.querySelector(selector);
		if (element) {
			return withElement(<HTMLElement>element, offdom);
		} else {
			return null;
		}
	}
	...
	public matches(selector: string): boolean {
		...
		const matches = (<any>element).webkitMatchesSelector || (<any>element).mozMatchesSelector || (<any>element).msMatchesSelector || (<any>element).oMatchesSelector;
		...
		if (current && current.children) {
			for (let i = 0, length = current.children.length; i < length; i++) {
				...
				if (hasData(<HTMLElement>element)) {
					...
					// Listeners
					const listeners: IDisposable[] = data(<HTMLElement>element)[LISTENER_BINDING_ID];
	...
	public destroy(): void {
		...
		if (this.currentElement) {
			...
			if (hasData(this.currentElement)) {
				...
				// Listeners
				const listeners: IDisposable[] = data(this.currentElement)[LISTENER_BINDING_ID];
				if (types.isArray(listeners)) {
					while (listeners.length) {
						listeners.pop().dispose();
					}
	...
	public getPositionRelativeTo(element: any): Box {
		...
		const left = DOM.getRelativeLeft(this.currentElement, element);
		const top = DOM.getRelativeTop(this.currentElement, element);
	...
	public getPosition(): Box {
		const position = DOM.getTopLeftOffset(this.currentElement);
...
export class MultiBuilder extends Builder {
	...
	public toArray(): Builder[] {
		return this.builders.concat();
	}
