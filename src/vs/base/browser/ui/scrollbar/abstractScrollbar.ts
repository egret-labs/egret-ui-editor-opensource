...
export abstract class AbstractScrollbar extends Widget {
	...
	public delegateMouseDown(browserEvent: MouseEvent): void {
		const e = new StandardMouseEvent(browserEvent);
	...
	private _onMouseDown(e: IMouseEvent): void {
		...
		const desiredSliderPosition = this._mouseDownRelativePosition(e, domNodePosition) - this._scrollbarState.getArrowSize() - this._scrollbarState.getSliderSize() / 2;
	...
	private _sliderMouseDown(e: IMouseEvent): void {
		if (e.leftButton) {
			...
			const initialScrollPosition = this._getScrollPosition();
			const draggingDelta = this._sliderMousePosition(e) - this._scrollbarState.getSliderPosition();
			...
			this._mouseMoveMonitor.startMonitoring(
				...
				(mouseMoveData: IStandardMouseMoveEventData) => {
					...
					if (Platform.isWindows && mouseOrthogonalDelta > MOUSE_DRAG_RESET_DISTANCE) {
						...
						const desiredSliderPosition = this._sliderMousePosition(mouseMoveData) - draggingDelta;
						this.setDesiredScrollPosition(this._scrollbarState.convertSliderPositionToScrollPosition(desiredSliderPosition));
					}
				},
				() => {
	...
	public setDesiredScrollPosition(desiredScrollPosition: number): boolean {
		...
		const oldScrollPosition = this._getScrollPosition();
		this._setScrollPosition(desiredScrollPosition);
		const newScrollPosition = this._getScrollPosition();
