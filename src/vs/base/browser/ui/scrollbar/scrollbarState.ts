...
export class ScrollbarState {
	...
	private _refreshComputedValues(): void {
		...
		const computedRatio = scrollSize > 0 ? (computedRepresentableSize / scrollSize) : 0;
		const computedIsNeeded = (scrollSize > visibleSize);
		...
		if (!computedIsNeeded) {
			...
			if (computedSliderSize < MINIMUM_SLIDER_SIZE) {
				...
				const sliderArtificialOffset = (MINIMUM_SLIDER_SIZE - computedSliderSize) / 2;
