...
export class LcsDiff2 {
	...
	private ComputeUniqueIdentifiers() {
		const xLength = this.x.getLength();
		const yLength = this.y.getLength();
		...
		for (i = 0; i < xLength; i++) {
			const xElementHash = this.x.getElementHash(i);
		...
		for (i = 0; i < yLength; i++) {
			const yElementHash = this.y.getElementHash(i);
	...
	private findCut(xStart: number, xStop: number, yStart: number, yStop: number, middle: number): number {
		const L1 = this.forward(xStart, middle, yStart, yStop);
		const L2 = this.backward(middle + 1, xStop, yStart, yStop);
	...
	private execute(xStart: number, xStop: number, yStart: number, yStop: number) {
		...
		if (xStart === xStop) {
			...
			const middle = Math.floor((xStart + xStop) / 2);
			const cut = this.findCut(xStart, xStop, yStart, yStop, middle);
