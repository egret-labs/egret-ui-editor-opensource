...
function createMapping(fill1: (map: string[]) => void, fill2: (reverseMap: IReverseMap) => void): Mapping {
	...
	const FINAL_REVERSE_MAP: IReverseMap = {};
	for (const entry in REVERSE_MAP) {
...
const STRING = createMapping((TO_STRING_MAP) => {
...
export const USER_SETTINGS = createMapping((TO_USER_SETTINGS_MAP) => {
...
class BinaryKeybindings {
	...
	public static isModifierKey(keybinding: number): boolean {
		...
		const keyCode = this.extractKeyCode(keybinding);
		return (
