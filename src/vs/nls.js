...
(function (NLSLoaderPlugin) {
    ...
    var NLSPlugin = (function () {
        ...
        NLSPlugin.prototype.create = function (key, data) {
			if (this.metadata && this.metadata.messages[key]) {
				return {
					localize: createScopedLocalize(this.metadata.messages[key])
				};
			} else {
				return {
        ...
        NLSPlugin.prototype.load = function (name, req, load, config) {
            ...
            if (!name || name.length === 0) {
				...
				if (pluginConfig.locale == 'zh-cn' && pluginConfig.metadata) {
					this.metadata = pluginConfig.metadata;
				}
				if (this.metadata && this.metadata.messages[name]) {
					load({
						localize: createScopedLocalize(this.metadata.messages[name])
					});
				}
                else if (Resources && Resources.getString) {
                    suffix = '.nls.keys';
